...
const main = async () => {
  ...
  for (const entry of entries) {
    ...
    const written: any = await payload.findByID({ collection: 'solutions', id, locale: 'tr', depth: 0 })
    const english = side(entry.en) as any
    english.capabilities = english.capabilities.map((cap: any, index: number) => ({
      ...cap,
      id: written.capabilities?.[index]?.id,
    }))

    await payload.update({ collection: 'solutions', id, locale: 'en', data: english as never })
